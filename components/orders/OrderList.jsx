...
import { CreditCard, Plus, Search, Eye, MessageCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function OrderList({ basePath }) {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get('/api/orders/list', { headers, params: { status, q, page, limit: 15 } });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setBranchWhatsapp(data.settings?.whatsappNumber || data.settings?.phone || '');
      } catch { /* non-fatal */ }
    })();
  }, []);

  const handleWhatsApp = (order) => {
    sendWhatsAppConfirmation(order, branchWhatsapp);
  };
  ...
  return (
    <div className="px-3 sm:px-6 py-4 sm:py-6 pb-28">
      ...
      {loading ? <Loading /> : orders.length === 0 ? (
        <div className="text-center py-16 bg-slate-50 rounded-xl border border-slate-200 text-slate-400">No payments found</div>
      ) : (
        <>
                ...
                <tbody>
                  {orders.map((o) => (
                    <tr key={o.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                      ...
                      <td className="px-5 py-3 text-slate-500 text-xs">{o.orderItems.map((i) => i.name).join(', ')}</td>
                      <td className="px-5 py-3 font-medium text-slate-700">₹{o.total.toLocaleString('en-IN')}</td>
                      <td className="px-5 py-3 text-slate-500 text-xs">{o.paymentMethod}</td>
                      <td className="px-5 py-3"><span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[o.status] || 'bg-slate-100 text-slate-500'}`}>{o.status}</span></td>
