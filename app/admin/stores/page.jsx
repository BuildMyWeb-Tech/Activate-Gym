    ...
    <div className="text-slate-500 mb-28 pt-4 md:p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl md:text-3xl flex items-center gap-2">
          <span className="bg-red-50 text-red-600 p-2 rounded-md inline-flex"><Building2Icon size={20} /></span>
          All <span className="text-slate-800 font-medium ml-1">Branches</span>
        </h1>
        <div className="flex gap-2">
          <span className="bg-red-50 text-red-600 px-3 py-1 rounded-full text-xs font-medium">{branches.filter((b) => b.isActive).length} Active</span>
          ...
          {branches.map((branch) => (
            <div key={branch.id} className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 md:p-6 flex max-md:flex-col gap-5 md:items-center max-w-4xl hover:shadow-md transition-shadow">
                  ...
                  {editingCommission === branch.id ? (
                    <div className="flex items-center gap-2">
                      <input type="number" min="0" max="100" step="0.5" value={commissionInput}
                        ...
                        className="border border-slate-300 rounded px-2 py-0.5 text-sm w-20 outline-none focus:ring-2 focus:ring-red-100" />
                      <span className="text-sm text-slate-500">%</span>
                      <button onClick={() => saveCommission(branch.id)} className="text-xs bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">Save</button>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className={`flex items-center gap-3 p-1.5 px-3 rounded-full ${branch.isActive ? 'bg-red-50' : 'bg-slate-100'}`}>
